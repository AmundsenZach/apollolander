...
function updateView() {
	...
	if (actionRectangle.height > 2000) {
		actionRectangle.height = 2000;
	}
	...
	for(var i = 0; i < lines.length; i++ ) {
		...
		if (!((right < line.p1.x) || (left > line.p2.x))) {
			...
			if(line.landable) {
				...
				if( pointIsLessThanLine(lander.bottomLeft, line.p1, line.p2) || pointIsLessThanLine(lander.bottomRight, line.p1, line.p2)) {
					setCrashed();
				}
			}
		}
